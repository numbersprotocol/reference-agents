"""status.py - Quick live status for public reference agents."""
...
for agent in AGENTS:
    ...
    try:
        with open(log_path, encoding="utf-8") as f:
            lines = f.readlines()
        count = sum(1 for line in lines if '"HTTP/1.1 201 Created"' in line)
        last_nid = ""
        for line in reversed(lines):
            if "registered  nid=" in line:
                last_nid = line.split("nid=")[1].split()[0]
                break
        grand_total += count
...
try:
    response = httpx.get("https://mainnet.num.network/api/v2/stats", timeout=8)
    data = response.json()
    total_tx = data.get("total_transactions", "n/a")
    today_tx = data.get("transactions_today", "n/a")
    wallets = data.get("total_addresses", "n/a")
    print(
        f"  Total transactions : {int(total_tx):,}"
        if str(total_tx).isdigit()
        else f"  Total transactions : {total_tx}"
    )
    print(f"  Transactions today : {today_tx}")
    print(
        f"  Total wallets      : {int(wallets):,}"
        if str(wallets).isdigit()
        else f"  Total wallets      : {wallets}"
    )
